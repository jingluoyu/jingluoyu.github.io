!function(e) {
	e.fn.slideCarsousel=function(t){
		...
		s= {
			...
			itemClassArr:[],init:function(){
				for(var e=0,t=s.count;
					e<t;e++)s.itemClassArr.push("item"+e);s.slideAutoChange()
			},
		slideAutoChange:function() {
			t.isAutoChange&&(t.slideInterval=setInterval(function(){s.toNext()
			},t.direction))
		},toNext:function() {
			...
			var i=t-s.currentIndex;if(0!=i){if(s.currentIndex=t,i>0){n=s.itemClassArr.splice(s.itemClassArr.length-i);return s.itemClassArr=e.merge(n,s.itemClassArr),void s.resetItemClass()
			}

			if(i<0) {
				var n=s.itemClassArr.splice(0,-i);return s.itemClassArr=e.merge(s.itemClassArr,n),void s.resetItemClass()
			}}},resetItemClass:function() {
				e.each(s.liList,function(t,i){e(i).removeClass().addClass(s.itemClassArr[t])
			...
			switch(s.init(),s.itemNext.click(function() {
				...
				case"click": s.indicatorList.click(function(){s.processIndicatorEvent(e(this).attr("data-slide-index"))
			});

				break;
				case"mouseover":s.indicatorList.mouseover(function() {
					s.processIndicatorEvent(e(this).attr("data-slide-index"))
				})}

				switch(t.slideType) {
					case"3d": s.ul.on("click",".item1 img",function(){s.toPrev()
					...
					s.slideCarousel.mouseover(function() {
						...
						slideInterval: "slideInterval",isAutoChange:!0,direction:5e3,callbackFunc:null,indicatorEvent:"click",slideType:"2d"
					}}(jQuery);